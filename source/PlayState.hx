...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'wow':
			dialogue = CoolUtil.coolTextFile(Paths.txt('wow/dialog'));
			case 'hoodie':
			dialogue = CoolUtil.coolTextFile(Paths.txt('hoodie/dialog'));
			case 'quake':
			dialogue = CoolUtil.coolTextFile(Paths.txt('quake/dialog'));
			case 'demise':
			dialogue = CoolUtil.coolTextFile(Paths.txt('demise/dialog'));
		}
	...
	{
		var black:FlxSprite = new FlxSprite(-200, -100).makeGraphic(FlxG.width * 3, FlxG.height * 3, FlxColor.BLACK);
		black.scrollFactor.set();
		add(black);
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					{
						LoadingState.loadAndSwitchState(new VideoState("assets/videos/trueending.webm",new StoryMenuState()));
				...
				else
				{
					...
					if (SONG.song.toLowerCase() == 'wow')
					{
						LoadingState.loadAndSwitchState(new VideoState("assets/videos/canthisbefixedpls.webm",new PlayState()));
					}
					if (SONG.song.toLowerCase() == 'hoodie')
					{
						LoadingState.loadAndSwitchState(new VideoState("assets/videos/slycutsceneoptomized.webm",new PlayState()));
					}
					if (SONG.song.toLowerCase() == 'quake')
					{
						LoadingState.loadAndSwitchState(new VideoState("assets/videos/canthisbefixedpls.webm",new PlayState()));
					}
	...
	if (SONG.song.toLowerCase() == 'demise') 
{
	...
	if (SONG.song.toLowerCase() == 'resonance') 
{
	if(curStep == 191)
	{
	...
	if (curStep == 255)
	{
		...
		remove(boyfriend);
		remove(gf);
		var bg:FlxSprite = new FlxSprite(-1000, -500).loadGraphic(Paths.image('resonancebg3','shared'));
		bg.antialiasing = true;
		bg.scrollFactor.set(0.2, 0.2);
		bg.active = false;
		bg.setGraphicSize(Std.int(bg.width * 0.8));
